...
public class Main {
    ...
    public static boolean checkForPassword(String str, int minLength) {
        if (str == null) {
            return false; // manually check null since Pattern.matches(String regex, CharSequence input) expects input to be non null
        }
    ...
    public static List<String> extractEmails(String str) {
        ...
        if (str == null) {
            return result; // manually empty list for null input
        }
    ...
    public static boolean checkForDoubles(String str) {
        ...
        return str.matches(".*([A-Z]).*\\1.*"); // str.matches matches the whole string, so it's like if it were anchored anyways
    }
}
